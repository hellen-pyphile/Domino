...
static void jogarPartida(Partida *partida, int jogadorComp) {
    ...
    while(jogo == 1) {
        ...
        if(contraComputador && partida->turno == 2) {
            mostrarTurno(partida->turno, contraComputador);
        ...
        if(escolha == 0) {
            ...
            if(passarTurno(partida, partida->turno) == 1) {
                ...
                mostrarMensagem("Nao e possivel passar a vez.\n");
            }
            ...
            indicePeca = solicitarIndicePeca();
            lado = solicitarLado();
...
/*
** Controla o turno do computador, realizando uma jogada valida ou
** comprando pecas ate encontrar uma jogada possivel
** Parametros:
...
void executarTurnoComputador(Partida *partida) {
    ...
    while(1) {
        ...
        if(indicePeca != -1) {
            ...
            mostrarJogadaComputador(esqJogada, dirJogada, lado);

            break;
        }
        else {
            if(comprarPeca(partida, 2) == 1) {
                mostrarCompraComputador();
            }
            else {
                mostrarPasseComputador();
                break;
            }
        }
    }
